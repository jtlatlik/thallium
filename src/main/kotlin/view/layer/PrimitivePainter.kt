...
//        }
    ...
    override fun visitVia(via: Via) {
        gc.save()
        val (x1,y1) = Point(via.center.x - 0.5*via.diameter, via.center.y - 0.5*via.diameter)
        val (x2,y2) = Point(via.center.x - 0.5*via.holeSize, via.center.y - 0.5*via.holeSize)
        gc.fill = Color.SILVER
        gc.fillOval(x1, y1, via.diameter, via.diameter)
        gc.fill = Color.CHOCOLATE
        gc.fillOval(x2,y2, via.holeSize, via.holeSize)
